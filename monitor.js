...
function renderAlerts() {
  ...
  const alerts = [];
  if (settings.last_measured_at && settings.feed_level <= 20) alerts.push({ type: "feed", title: "Feed level is low", text: "Refill the feed container soon." });
...
function renderFeed() {
  ...
  const measuredGrams = Number(settings.feed_weight_g || 0);
  const capacityGrams = Number(settings.feed_capacity_g || 10000);
  $("foodLevel").textContent = settings.last_measured_at ? formatFeed(measuredGrams) : "Waiting for scale";
  $("foodLabel").textContent = settings.last_measured_at ? `${settings.feed_level}% of ${formatFeed(capacityGrams)} capacity` : "Live scale measurement";
  $("foodProgress").style.width = `${settings.last_measured_at ? settings.feed_level : 0}%`;
  $("feedStatus").textContent = settings.last_measured_at ? "Measured by ESP32 scale" : "Waiting for ESP32";
  if ($("foodMeasuredAt")) $("foodMeasuredAt").textContent = settings.last_measured_at ? `Updated ${new Date(settings.last_measured_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", second: "2-digit" })}` : "";
...
async function init() {
  ...
  if ($("feedNowBtn")) $("feedNowBtn").addEventListener("click", async () => { await addActivity("Manual feeding requested", "Awaiting feeder hardware", "feed"); toast("Feeding request recorded. The scale reading will update after dispensing."); });
